...
function myaacAuthenticateLoginRequest(object $request): array
{
	...
	else if($inputAccountName != false) {
	...
	$currentPassword = encrypt((USE_ACCOUNT_SALT ? $account->salt : '') . $inputPassword);
	...
	setSession([
		...
		'remember_me' => !empty($request->stayloggedin) ? true : null,
		'last_visit' => time(),
	]);
	if (fieldExist('web_lastlogin', 'accounts')) {
		$account->web_lastlogin = time();
		$account->save();
	}
	...
	return [
		...
		'token' => $inputToken,
		'accountHasSecret' => $accountHasSecret,
...
switch ($action) {
	...
	case 'paneltoken':
		$auth = myaacAuthenticateLoginRequest($request);
		$redirect = myaacPanelAuthNormalizeRedirect($request->redirect ?? null);
		$token = myaacPanelAuthIssue([
			'account_id' => (int)$auth['account']->id,
			'redirect' => $redirect,
			'remember_me' => !empty($request->stayloggedin),
			'ip' => get_browser_real_ip(),
		]);

		die(json_encode([
			'panel_url' => '/panel-auth.php?token=' . urlencode($token),
		]));
	...
	case 'login':
		...
		$inputAccountName = $auth['accountname'];
		$inputToken = $auth['token'];
		$accountHasSecret = $auth['accountHasSecret'];
