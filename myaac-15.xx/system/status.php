...
if($settingPort !== false && $settingPort !== null && $settingPort !== '') {
	$status_port = (int)$settingPort;
}
elseif(!isset($status_port) || $status_port === '') // try 7171 if no port specified
{
	$status_port = 7171;
}
else {
...
function updateStatus() {
	...
	else
	{
		...
		$uptime = $status['uptime'] = (int)$serverStatus->getUptime();
		$days = intdiv($uptime, 86400);
		$hours = intdiv($uptime % 86400, 3600);
		$minutes = intdiv($uptime % 3600, 60);
		$status['uptimeReadable'] = ($days > 0 ? $days . 'd ' : '') . $hours . 'h ' . $minutes . 'm';
