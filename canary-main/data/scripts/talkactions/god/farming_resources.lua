...
function farmingResources.onSay(player, words, param)
	...
	if material == "all" then
		...
		player:sendTextMessage(MESSAGE_ADMINISTRATOR, string.format("Added %d wood and %d stone.", amount, amount))
		return true
	end
	...
	player:sendTextMessage(MESSAGE_ADMINISTRATOR, string.format("Added %d %s.", amount, material))
	return true
end

farmingResources:separator(" ")
farmingResources:groupType("god")
